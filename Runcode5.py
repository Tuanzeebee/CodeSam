...
if file is not None:
    ...
    # ============================
    # Người dùng chọn giả định để suy ngược thời gian học
    # ============================
    st.markdown("### 🧮 Chọn giả định để suy ngược thời gian học từ điểm thực tế")
    default_attendance = st.slider("📊 Chọn tỷ lệ chuyên cần giả định (%)", min_value=50, max_value=100, value=75)
    default_support = st.selectbox("👪 Chọn mức hỗ trợ gia đình giả định", ["thấp", "trung bình", "cao", "rất cao"], index=1)
    support_mapping = {"thấp": 0, "trung bình": 1, "cao": 2, "rất cao": 3}
    support_val = support_mapping[default_support]

    # Nếu có điểm thực tế thì tính ngược thời gian học ước lượng
    if 'raw_score' in df.columns:
        df['attendance_float'] = default_attendance
        df['family_support'] = support_val
        df['estimated_weekly_study_hours'] = df['raw_score'] / (default_attendance / 100 + 0.1 * (support_val + 1))
        st.markdown("#### 📌 Ước lượng thời gian học từ điểm thực tế")
        st.dataframe(df[['course_code', 'raw_score', 'estimated_weekly_study_hours']])

    # ============================
    # Chọn phương thức nhập thông tin bổ sung
    # ============================
    ...
    df['semester_number'] = df['semester_number'].apply(
        lambda x: 3 if str(x).strip().lower() == 'hè' else pd.to_numeric(x, errors='coerce'))
    ...
    if missing_cols:
        ...
        try:
            ...
            with tab1:
                ...
                st.download_button("📥 Tải kết quả về (.csv)", data=csv, file_name="ket_qua_du_doan.csv",
                                   mime="text/csv")
            ...
            with tab2:
                ...
                if 'weekly_study_hours' in df.columns and 'student_id' in df.columns:
                    study_sum = df[['student_id', 'weekly_study_hours']].groupby('student_id', as_index=False).sum()
